...
Ext.define('Ext.ux.form.field.CodeMirror', {
    ...
    showTheme : true,
    addons : [],
    addonPath : 'js/common/ux/CodeMirror/addon/',
    /**
    ...
    */
    pathModes:          'js/common/ux/CodeMirror/mode',
    ...
    */
    ...
    initComponent : function(){
        ...
        if (!Ext.Object.isEmpty(me.addons)) {
	        Ext.each(me.addons,function(addon){
	            if (Ext.isObject(addon) && !Ext.Object.isEmpty(addon)) {
	                jsFiles = jsFiles.concat(Ext.value(addon.jsFiles,[]));
	                cssFiles = cssFiles.concat(Ext.value(addon.cssFiles,[]));
	                delete addon.jsFiles;
	                delete addon.cssFiles;
	                Ext.apply(me,addon);
	            }
	        });
	        Ext.each(jsFiles,function(file){
	            Ext.Loader.loadScript({
	                url : me.addonPath + file
	            });
	        });
	        
	        Ext.each(cssFiles,function(file){
	            Ext.util.CSS.swapStyleSheet(Ext.id(),me.addonPath+file);
	        });            
        }
    ...
    */
    getSubTplData: function() {
        ...
        Ext.apply(ret,{
            size : Ext.String.format('height:{0}px;width:100%',me.grow ? me.growMin : Ext.value(me.height,100))
        });
    ...
    */
    afterRender: function() {
        var me = this,
            personConfig = me.getPersonConfig();
            
        me.theme = Ext.value(personConfig.theme,me.theme);
    ...
    */
    createToolbar : function(editor){
        ...
        if (me.showTheme) {
            ...
            themesSelectItem = Ext.widget('component', {
                renderTpl: [
                    '<select id="{id}-selectEl" class="{cls}">',
                        '<tpl for="themes">',
                            '<option value="{.}" <tpl if="this.isSelected(values)"> selected</tpl>>{.}</option>',
                        '</tpl>',
                    '</select>',{
                        theme: me.theme,
                ...
                listeners: {
                    change: function() {
		                var theme = me.themesSelect.dom.value;
    ...
    updateToolbarButtons: function(){
        ...
        try{
            ...
            if(me.showLineNumbers){
                btns['insertorderedlist'].toggle(me.showLineNumbers);
            }
        }catch(err){
    ...
    getPersonConfig : function(){
        ...
        return Ext.isEmpty(v) ? '' : Ext.decode(v);    
    },
    /**
    ...
    switchTheme : function(theme){
        ...
        if(!me.cacheTheme[theme] && theme!=='default'){
            Ext.util.CSS.swapStyleSheet(theme,'./js/common/ux/CodeMirror/theme/'+theme+'.css');
            ...
            if(!Ext.Array.contains(me.scriptsLoaded, path + '/' + item.dependencies[i])){
                var options = {
                    ...
                    onLoad: function(options){
                        var ok = true;
                        for(var j=0; j < me.scripts.length; j++){
    ...
    */
    ...
    autoSize: function() {
        ...
        if (me.grow && me.rendered) {
            me.updateLayout();
            height = me.editorEl.getHeight();
            
//            height = Math.min(me.growMax, Math.max(height, me.growMin)) ;
